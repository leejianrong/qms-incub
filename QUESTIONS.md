...
| Q36 | Embedding model for RAG ingestion/retrieval (S6/S8) | DECIDED | *(revised)* Provider-swappable via `EMBEDDING_PROVIDER` (`local`/`openrouter`/`zenmux`), same OpenAI-compatible-client pattern as Q37's `LLM_PROVIDER`. Default stays `local` (HuggingFace `BAAI/bge-small-en-v1.5` via LlamaIndex's `HuggingFaceEmbedding`, run in-process, no API key, offline-capable) — hosted providers exist for a machine you'd rather not run a local model on at all. Correction to this row's original premise: OpenRouter now exposes an OpenAI-compatible `/embeddings` endpoint (it didn't when this was first decided), so "OpenRouter is chat-only" no longer holds; ZenMux also proxies embedding models but per Q39 is a time-boxed provider, not a durable choice for anything that must keep working after its window lapses | ADR-0009 (unchanged), V1 implementation |
