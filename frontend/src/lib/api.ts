...
export async function startBatch(
  ...
  const response = await fetchImpl(`${apiBase}/documents/batch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  return (await response.json()) as BatchStarted;
}
...
export async function fetchDocuments(
  apiBase: string,
  fetchImpl: typeof fetch = fetch,
): Promise<PolicyDocumentStatus[]> {
  const response = await fetchImpl(`${apiBase}/documents`);
  if (!response.ok) {
    throw new Error(`backend returned ${response.status}`);
  }
  return (await response.json()) as PolicyDocumentStatus[];
}
